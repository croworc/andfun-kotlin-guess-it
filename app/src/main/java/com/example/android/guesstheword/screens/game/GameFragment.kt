...
class GameFragment : Fragment() {
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?,
        ...
        // COMPLETED (04) Update these onClickListeners to call methods in the ViewModel, then
        // update the UI
        binding.correctButton.setOnClickListener {
            viewModel.onCorrect() // increments the counter, retrieves the next word to guess
            updateScoreText()
            updateWordText()
        }

        binding.skipButton.setOnClickListener {
            viewModel.onSkip() // decrements the counter, also retrieves the next word to guess
            updateScoreText()
            updateWordText()
        }

        // Sets the initial UI state for the score to 0 and for the word to ""
        updateScoreText()
        updateWordText()
